...
| Microsoft callback shows failed        | Do not retry repeatedly. Inspect only the sanitized correlation ID, requested-scope envelope, failure class, and token/profile validation result; never log the code, verifier, client secret, token, callback URL, or provider body. |
