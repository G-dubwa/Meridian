...
| Column | SQL type | Null | Default | Primary |
|---|---|---|---|---|
| `manifest_id` | `uuid` | no | no | no |
| `user_id` | `uuid` | no | no | no |
| `ordinal` | `integer` | no | no | no |
| `evidence_lane` | `text` | no | no | no |
| `source_kind` | `text` | yes | no | no |
| `resource_id` | `uuid` | yes | no | no |
| `entry_revision_id` | `uuid` | yes | no | no |
| `knowledge_chunk_id` | `uuid` | yes | no | no |
| `knowledge_source_revision_id` | `uuid` | yes | no | no |
| `content_hash` | `text` | yes | no | no |
| `methods` | `text[]` | no | yes | no |
| `score` | `numeric(10, 8)` | yes | no | no |
| `policy_reference` | `text` | yes | no | no |

Foreign keys:
- `context_manifest_items_user_id_users_id_fk`: (user_id) → `users` (id)
- `context_manifest_items_manifest_owner_fk`: (manifest_id, user_id) → `context_manifests` (id, user_id)
- `context_manifest_items_resource_owner_fk`: (resource_id, user_id) → `resources` (id, user_id)
- `context_manifest_items_entry_owner_fk`: (entry_revision_id, user_id) → `entry_revisions` (id, user_id)
- `context_manifest_items_chunk_owner_fk`: (knowledge_chunk_id, user_id) → `knowledge_chunks` (id, user_id)
- `context_manifest_items_revision_owner_fk`: (knowledge_source_revision_id, user_id) → `knowledge_source_revisions` (id, user_id)

Indexes: `context_manifest_items_user_manifest_idx`.
...
| `policy_version` | `text` | no | no | no |
| `semantic_retrieval_active` | `boolean` | no | yes | no |
| `created_at` | `timestamp with time zone` | no | yes | no |

Foreign keys:
- `context_manifests_user_id_users_id_fk`: (user_id) → `users` (id)

Indexes: `context_manifests_user_created_idx`.
...
| `user_id` | `uuid` | no | no | no |
| `lane` | `text` | no | no | no |
| `source_kind` | `text` | no | no | no |
| `entry_revision_id` | `uuid` | yes | no | no |
| `knowledge_chunk_id` | `uuid` | yes | no | no |
| `content_hash` | `text` | no | no | no |
| `model_id` | `text` | no | no | no |
| `model_version` | `text` | no | no | no |
| `dimensions` | `integer` | no | no | no |
| `embedding` | `vector` | no | no | no |
| `created_at` | `timestamp with time zone` | no | yes | no |

Foreign keys:
- `retrieval_embeddings_user_id_users_id_fk`: (user_id) → `users` (id)
- `retrieval_embeddings_entry_owner_fk`: (entry_revision_id, user_id) → `entry_revisions` (id, user_id)
- `retrieval_embeddings_chunk_owner_fk`: (knowledge_chunk_id, user_id) → `knowledge_chunks` (id, user_id)

Indexes: `retrieval_embeddings_user_lane_model_idx`.
